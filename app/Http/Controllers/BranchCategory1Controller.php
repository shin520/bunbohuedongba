...
use Carbon\Carbon;
class BranchCategory1Controller extends Controller
{
    public function index(){
        $data = BranchCategory1::orderby('number','asc')->orderby('id','desc')->get();
        return view('be.branch_category_1.index',compact('data'));
    }
    public function add(){
        return view('be.branch_category_1.create');
    }
    public function store(Request $request){
        ...
        $validated = $request->validate([
            ...
            'slug' => "required|unique:branch_category1s,slug"
        ],$trans);
        $data = new BranchCategory1();
        if($request->file('img')){
        ...
        return redirect()->route('be.branch_category_1.index');
    }
    public function edit($id){
        $data = BranchCategory1::find($id);
        return view('be.branch_category_1.edit',compact('data'));
    }
    public function update($id,Request $request){
        ...
        $validated = $request->validate([
            ...
            'img' => 'image|mimes:jpeg,png,jpg,gif,svg|max:2048',
            'file' => 'max:10240|mimes:zip,pdf,jpeg,png,jpg',
            'slug' => "required|unique:branch_category1s,slug,$id,id"
        ],$trans);
    ...
    public function featured(Request $request){
        $data = BranchCategory1::find($request->id);
        $data->featured = $request->featured;
        $data->save();
