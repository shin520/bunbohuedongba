...
class GalleryTypeController extends Controller
{
    public function index(){
        $data = GalleryType::orderby('number','asc')->orderby('id','desc')->get();
        return view('be.gallerytype.index',compact('data'));
    }
    public function add(){
        return view('be.gallerytype.create');
    }
    public function store(Request $request){
        ...
        $validated = $request->validate([
            ...
            'slug' => "required|unique:gallery_types,slug"
        ],$trans);
        $data = new GalleryType();
        if($request->file('img')){
...
;
        ...
        return redirect()->route('be.gallerytype.index');
    }
    public function edit($id){
        $data = GalleryType::find($id);
        return view('be.gallerytype.edit',compact('data'));
    }
    public function update($id,Request $request){
        ...
        $validated = $request->validate([
            ...
            'img' => 'image|mimes:jpeg,png,jpg,gif,svg|max:2048',
            'file' => 'max:10240|mimes:zip,pdf,jpeg,png,jpg',
            'slug' => "required|unique:gallery_types,slug,$id,id"
        ],$trans);
        $data = GalleryType::find($id);
        if($request->file('img')){
            $file= $request->file('img');
    ...
    public function featured(Request $request){
        $data = GalleryType::find($request->id);
