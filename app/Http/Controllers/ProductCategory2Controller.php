...
use Carbon\Carbon;
class ProductCategory2Controller extends Controller
{
    public function index(Request $request){
        $parent = ProductCategory1::get();
        $keyword = $request->search ?? false;
        if($request->search){
            $data = ProductCategory2::where('name', 'LIKE', "%{$request->search}%")->paginate(10);
        }
        else{
        $data = ProductCategory2::with('parent')->orderby('number','asc')->orderby('id','desc')->paginate(10);
        }
        return view('be.product_category_2.index',compact('data','parent','keyword'));
    }
    public function add(){
        $parent = ProductCategory1::get();
        return view('be.product_category_2.create',compact('parent'));
    }
    public function store(Request $request){
        ...
        $validated = $request->validate([
            ...
            'slug' => "required|unique:product_category2s,slug"
        ],$trans);
        ...
        return redirect()->route('be.product_category_2.index');
    }
    public function edit($id){
        $parent = ProductCategory1::get();
        $data = ProductCategory2::find($id);
        return view('be.product_category_2.edit',compact('data','parent'));
    }
    public function update($id,Request $request){
        ...
        $validated = $request->validate([
            ...
            'img' => 'image|mimes:jpeg,png,jpg,gif,svg|max:2048',
            'file' => 'max:10240|mimes:zip,pdf,jpeg,png,jpg',
            'slug' => "required|unique:product_category2s,slug,$id,id"
        ],$trans);
        ...
        $link = Product::where('parent_child_id',$id)->get();
            foreach($link as $l){
    ...
    public function featured(Request $request){
        $data = ProductCategory2::find($request->id);
        $data->featured = $request->featured;
        $data->save();
    ...
    public function update_parent(Request $request){
        ...
        $data = ProductCategory2::find($id_flink_child_parent);
        $data->parent_id = $id_parent;
        $product = Product::where('parent_child_id',$id_flink_child_parent)->get();
        $data->save();
